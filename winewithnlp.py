...
# NLP Insight 7: Most frequent country in the dataset
...
# Determine the most liked wine variety based on average points
def most_liked_variety_by_points(df_test, model, vectorizer, le_variety):
    logging.info("Predicting wine varieties for the test set...")

    # Vectorize and add features for the test set
    X_test = vectorize_and_add_features(df_test, vectorizer)

    # Predict wine variety
    predicted_varieties_encoded = model.predict(X_test)
    df_test['predicted_variety'] = le_variety.inverse_transform(predicted_varieties_encoded)

    # Group by predicted wine variety and calculate the average points for each variety
    most_liked_variety = df_test.groupby('predicted_variety')['points'].mean().idxmax()
    avg_points = df_test.groupby('predicted_variety')['points'].mean().max()

    print(f"Most liked wine variety (based on average points): {most_liked_variety} with an average of {avg_points:.2f} points.")
    return most_liked_variety
...
def main(train_path, test_path):
    ...
    # Predict the wine variety and find the most liked variety based on points
    most_liked_variety_by_points(df_test, model, vectorizer, le_variety)
